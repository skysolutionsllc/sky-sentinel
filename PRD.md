...
8. [Application 1: DME Claims Portal (Dummy Claim System)](#8-application-1-dme-claims-portal)
...
**Sky Sentinel** is a hackathon MVP that demonstrates how modern AI — specifically Large Language Models (LLMs) and advanced anomaly detection — can augment CMS's existing Fraud Prevention System (FPS) to proactively detect fraud, waste, and abuse (FWA) in Medicare Durable Medical Equipment (DME) claims.

CMS's current FPS, built circa 2018 using traditional machine learning, excels at **deterministic pattern matching** — finding statistical outliers based on predefined thresholds and rules. However, it struggles to detect **complex, evolving, and coordinated fraud schemes** that are specifically designed to fly under individual-supplier thresholds.

Sky Sentinel doesn't replace FPS. It **lives alongside it** as a complementary intelligence layer that:
...
- Generates **explainable, evidence-backed alerts** that tie directly to CMS program integrity workflows
...
Medicare fraud, waste, and abuse cost the U.S. government an estimated **$60+ billion annually**. DME fraud is one of the most targeted categories because:
...
### 2b. Limitations of the Current FPS

CMS's Fraud Prevention System uses traditional ML techniques (logistic regression, random forest, rule-based systems) that have known limitations:
...
| **Structured data only** | FPS analyzes billing codes and dollar amounts but cannot understand claim narratives, medical justifications, or policy language |
...
| **Reactive "pay and chase"** | Many improper payments are identified only after payment, leading to costly recovery efforts |
...
Sky Sentinel operates as a **complementary intelligence layer** alongside CMS's existing FPS:
...
│                                                                  │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────────┐ │
│  │  DME Claims   │────▶│  Existing    │────▶│  UCM (Unified    │ │
│  │  Portal       │     │  FPS System  │     │  Case Mgmt)      │ │
│  │  (Hackathon   │     │  (Trad. ML)  │     │                  │ │
│  │   Simulator)  │     │              │     │                  │ │
│  └──────┬───────┘     └──────────────┘     └──────────────────┘ │
│         │                                                        │
...
### How Sky Sentinel Complements FPS

| Capability | FPS (Traditional ML) | Sky Sentinel (LLM-Augmented AI) |
|---|---|---|
...
This is the **core thesis of Sky Sentinel** — that modern LLMs bring capabilities fundamentally unavailable to the 2018-era FPS. Here are the specific fraud patterns and detection methods LLMs unlock:

### 6a. Semantic Analysis of Claim Narratives

**What FPS does:** Looks at HCPCS codes and dollar amounts.
**What LLMs add:** Reads the actual text of claims — medical necessity justifications, physician notes, prior authorization language — and identifies inconsistencies.
...
**What FPS does:** Applies static coded rules.
**What LLMs add:** Ingests CMS Local Coverage Determinations (LCDs), National Coverage Determinations (NCDs), and DME MAC policies, then checks whether a claim's documentation actually satisfies the policy requirements.
...
**What FPS does:** Analyzes each supplier independently.
**What LLMs add:** Compares the text and language patterns across claims from multiple suppliers and flags when different NPIs are submitting suspiciously similar or templated justification language.
...
**What FPS does:** Generates a numeric risk score with limited explanation.
**What LLMs add:** Produces a **narrative risk assessment** that explains in plain English why a supplier or cluster is suspicious, what specific evidence supports the flag, and what an investigator should look for next.

> **Example output from Sky Sentinel:**
> *"DME Supplier NPI-1234567 is flagged as HIGH RISK (score: 87/100). Key concerns: (1) 340% increase in CPAP equipment billing over 90 days with no corresponding increase in referring physicians, (2) 94% of beneficiaries are located 100+ miles from the supplier's listed address, (3) Medical necessity justifications across 47 claims use nearly identical language suggesting templated documentation. Recommended action: Escalate to UCM for coordinated review with NPIs 2345678 and 3456789, which show similar behavioral patterns."*

### 6e. Emerging Scheme Detection Through Reasoning

**What FPS does:** Detects patterns it was trained on.
**What LLMs add:** Can **reason about novel patterns** by understanding the underlying logic of fraud — even patterns it hasn't seen before.
...
**What FPS provides:** Dashboard with predefined filters.
...
- **Action panel** — Classify as Valid Concern / False Positive / Monitor; add investigator notes; escalate to UCM
...
**Investigator action:** Reviews the alert, sees the evidence, classifies as Valid Concern, escalates to UCM with the AI-generated evidence package.

### Use Case 2: Coordinated Multi-NPI Billing Pattern (Inspired by 2025 DOJ Case)

**Scenario:** Based on the *largest healthcare fraud case by loss amount ever charged by the DOJ* — a criminal organization purchased dozens of DME companies, used nominee owners, and rapidly billed through those entities. When one was shut down, they replaced it with another.
...
- No single supplier exceeds traditional FPS thresholds individually
- LLM finds near-identical medical necessity language across all 7 NPIs
- Collective cluster risk score: 94/100

**Investigator action:** Uses HITL controls to test the cluster pattern against the broader dataset, finds 3 additional NPIs matching the pattern that weren't in the initial cluster. Escalates the full group.
...
> "FPS is great at catching this. A single supplier with a billing spike. But let me show you what it catches."
...
> "But here's what FPS misses. None of these 7 suppliers individually look suspicious. Each one is just below the threshold. But together..."
...
- Clicks "Escalate to UCM" — the investigation package is ready
...
|---|---|
| **Mission Alignment** | Directly addresses CMS Program Integrity's #1 challenge — proactive DME fraud detection. Designed to complement the existing FPS. |
...
2. **Problem Statement** — Why this matters (with data points)
...
- Integration with CMS FPS API for complementary alerting
- Integration with UCM for case creation
...
- Cross-agency data fusion (DOJ, OIG, FBI)
- Real-time beneficiary protection alerts
- Policy impact analysis — how LCDs/NCDs affect fraud patterns

---

> **Sky Sentinel: Moving CMS from reactive review to proactive prevention — where AI amplifies human expertise, not replaces it.**
