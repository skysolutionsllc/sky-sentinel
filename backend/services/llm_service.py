...
class AnthropicProvider(BaseLLMProvider):
    ...
    def analyze_supplier(self, supplier_data: dict, claims: list, peer_baseline: dict) -> str:
        system = (
            ...
            "In the EVIDENCE SUMMARY, include specific claim numbers from the data, "
            "service dates, HCPCS codes with dollar amounts, beneficiary counts, "
            "and peer comparison statistics to maximize investigative explainability."
        )
        prompt = (
...
class MockLLMProvider(BaseLLMProvider):
    ...
    def analyze_supplier(self, supplier_data: dict, claims: list, peer_baseline: dict) -> str:
        ...
        return (
            ...
            f"### EVIDENCE SUMMARY\n"
            f"- **Claim CLM-2025-{npi[-5:]}-047** (K0856, Power Wheelchair): $28,440 billed on 2025-01-14 — beneficiary located 340 miles from supplier address\n"
            f"- **Claim CLM-2025-{npi[-5:]}-051** (K0871, Wheelchair Accessory): $23,687 billed on 2025-01-22 — same referring physician as 6 other claims\n"
            f"- **Claim CLM-2025-{npi[-5:]}-063** (E0260, Hospital Bed): $4,215 billed on 2025-02-03 — medical necessity text 94% similar to CLM-047\n"
            f"- Billing growth: 280%+ over prior quarter vs. peer median of 5-8%\n"
            f"- Unique beneficiary states: 12 (peer avg: 1-2)\n"
            f"- Referring physicians: 2 physicians account for 89% of orders ({len(claims)} total claims sampled)\n"
            f"- HCPCS concentration: Top 3 codes represent 94% of ${sum(c.get('billed_amount', 0) for c in claims[:5]):,.0f}+ in sampled volume\n"
            f"- Peer baseline avg total billed: ${peer_baseline.get('avg_total_billed', 45000):,.0f} — this supplier: {peer_baseline.get('deviation_factor', '4.3')}x above peer mean\n\n"
