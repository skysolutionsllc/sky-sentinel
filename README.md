...
| **Diversified Evidence Tags** | Each supplier has unique, data-derived evidence: "Explosive claims growth: 100th percentile vs. peer group", "High-cost HCPCS concentration", "Extreme peer deviation vs. state mean", "Geographic impossibility indicator", "Linked to coordinated supplier network" |
...
| Control | Function |
|---|---|
| **6-Dimension Threshold Sliders + Risk Cutoff** | Adjustable sensitivity for: Billing Volume vs. Peers, Growth Rate, HCPCS Concentration, Geographic Spread, Peer Deviation (Z-Score), Cluster Association — plus a risk threshold cutoff. Each slider updates the alert population in real-time |
...
| `admin@skysolutions.com` | `sky123` | Admin (generic demo fallback) |

The login page accepts email addresses — use any of the team member emails above, or `admin@skysolutions.com` for the generic demo fallback.
...
| **Peer Deviation (Z-Score)** | 15% | Statistical peer analysis | How many standard deviations a supplier's billing deviates from its state peer group mean — a Z-score of 4+ indicates a statistical outlier |
...
- The 6-factor risk score is fully decomposable: investigators can see exactly how much billing volume, growth rate, HCPCS mix, geographic spread, peer deviation, and cluster association contribute to the final score
...
|---|---|---|
| **Billing anomaly types** | 6 named scoring factors: Billing Volume vs. Peers, Growth Rate, HCPCS Concentration, Geographic Spread, Peer Deviation (Z-Score), Cluster Association | Supplier Detail → Risk Factor Breakdown |
...
| **Combined score formula** | Weighted composite of Isolation Forest anomaly score, Z-score peer deviation, DBSCAN cluster membership, growth rate, HCPCS concentration, and geographic spread | Composite Risk Scoring section (README) |
